...
class Package extends CI_Controller
{
    ...
    public function laporanperbulan()//sesuaikan di list
	{
		...
		if (isset($_POST['cetaksemua'])) {
			...
			$this->data['package_item'] =  $this->package_m->get_all();//
			$this->data['title_web'] = 'Laporan Akta Lahir';	
		
		}
		
        $this->load->view('backend/package/item_doc',$this->data);
	}
    public function word()
    {
        $data = array(
            'item' => $this->package_m->get_all(),
            'start' => 0
        );
        
        $this->load->view('backend/package/item_doc',$data);
    }
}
